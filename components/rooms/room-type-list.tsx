  ...
  if (roomTypes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-white py-12">
        ...
        <h3 className="mt-4 text-lg font-semibold text-slate-900">{t.rooms.noRoomTypes}</h3>
        <p className="mt-2 text-sm text-slate-600">{t.rooms.createRoomTypeFirst}</p>
      </div>
    )
  }
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-4 text-sm text-slate-600">
              <div className="flex items-center gap-1">
                ...
                <span>{roomType.maxGuests} {t.rooms.maxGuests.toLowerCase()}</span>
              </div>
              <div className="flex items-center gap-1">
                <Bed className="h-4 w-4" />
                <span>{roomType._count.rooms} {t.rooms.rooms.toLowerCase()}</span>
              </div>
            </div>
